...
Legacy .doc/.xls/.ppt conversion itself now happens inline inside pii_triage.runner's shared
_process_file (LibreOffice headless, cross-platform -- see pii_triage_merged/tests/
test_process_file_conversion.py), so worker.py no longer has any Windows-leg-specific
forwarding logic of its own to test here.

No Azure needed: monkeypatches scaling_lib functions directly (same style test_hwe_scaled_ui.py
already uses). Point SCALING_LIB_SRC at a scaling-lib checkout for local runs where scaling-lib
isn't pip-installed.
