...
| B2 | ~~Windows leg is a manual native worker~~ — **resolved.** Legacy `.doc/.xls/.ppt` conversion now runs inline via LibreOffice headless (`conversion.py`) on the normal Linux fleet; no Windows VM, no separate queue, no manual start/supervise step. | `conversion.py`; `RUNBOOK.md` §3.5 | ✅ | LibreOffice conversion fidelity vs. the old Win32-COM path is not yet validated against a real corpus at scale — do that before fully retiring the rollback path (git history + `run_worker_forever.ps1`). |
