...
| ~~Start the Windows worker~~ | No longer applicable — legacy `.doc/.xls/.ppt` conversion runs inline via LibreOffice on the normal Linux fleet now (no Win32 COM, no separate machine). | Nothing to do; those files just flow through the same Container App workers as everything else. |
